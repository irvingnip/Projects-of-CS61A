...
def choose(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 1
    num_para = [para for para in paragraphs if select(para)]
    if k >= len(num_para):
        return ""
    else:
        return num_para[k]
...
def shifty_shifts(start, goal, limit):
    ...
    if start == goal:
        ...
        if start[0] == goal[0]:
            limit -= 0
            num_diff = 1
        return num_diff + shifty_shifts(start[1:], goal[1:], limit)
...
def pawssible_patches(start, goal, limit):
    ...
    if limit < 0:
        ...
        add_diff = pawssible_patches(start[1:], goal, limit-1)
...
def time_per_word(times_per_player, words):
    ...
    for tt in times_per_player:
        time = []
        for i in range(len(time) - 1):
            time.append(tt[i] - tt[i - 1])
        tpp.append(time)
