    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(place.ant):
                ContainerAnt.contain_ant(self, place.ant)
                place.ant = self
            elif place.ant.can_contain(self):
                ContainerAnt.contain_ant(place.ant, self)
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        curr_range = 0
        while True:
            if curr_place == beehive:
                return None
            ret = rANTdom_else_none(curr_place.bees)
            if ret is not None and self.min_range <= curr_range <= self.max_range:
                return ret
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if amount >= self.armor:
            to_damage = amount + self.damage
        else:
            to_damage = amount
        for b in self.place.bees[:]:
            Insect.reduce_armor(b, to_damage)
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt is an ant that does nothing each turn. A WallAnt is useful
    because it has a large armor value."""
    name = 'Wall'
    food_cost = 4
    implemented = True
    def __int__(self, armor = 4):
        Ant.__init__(self, armor)
...
class ScubaThrower(ThrowerAnt):
    """A subclass of ThrowerAnt that can be placed on Water."""
    name = 'Scuba'
    food_cost = 6
    is_watersafe = True
    implemented = True
    def __int__(self, armor = 1):
        Ant.__init__(self, armor)
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.is_real:
            ...
            while True:
                if curr_place == None:
                    break
                elif curr_place.ant != None and curr_place.ant.buffed == False:
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.is_real:
            bees_win()
        # END Problem EC
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant is None or self.place.ant.block_path is False:
            return False
        else:
            return True
